...
END:VCARD`;
    ...
    // ... rest of the download logic stays the same ...
    ...
    link.setAttribute(
      "download",
      `${data.name.replace(/\s+/g, "_")}_Contact.vcf`
    );
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden transform transition-all hover:shadow-3xl">
        ...
        <div className="p-8">
          <div className="space-y-4 mb-8">
            ...
            {/* Secondary Number (Only shows if mobile2 exists in json) */}
            {data.mobile2 && (
              <ContactRow
                icon={<Phone size={20} />}
                text={data.mobile2}
                href={`tel:${data.mobile2}`}
              />
            )}
            <ContactRow
              icon={<Mail size={20} />}
